...
group.add_argument("--restart", help="Restart the container without rebuilding", action='store_true')
...
def setup():
    ...
    results = cmd_run('sudo docker run --rm -it --entrypoint "./manage.py" {} "migrate"'.format(imageName))
    checkResults(results)
